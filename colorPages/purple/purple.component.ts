...
import {FioletComponent} from './fiolet/fiolet.component';
...
@Component({
  ...
  styleUrl: './purple.component.scss'
})
export class PurpleComponent implements AfterViewInit {
...
// read: HTMLParagraphElement - дополнительный этап фильтрации, т.е. можем указать селектор или название компонента, чтобы повторно отфильтровать то, что нам нужно будет найти
  ...
  // @ViewChild('component') childComponent?:FioletComponent;
  @ViewChild(FioletComponent) childComponent?:FioletComponent; // такая запись работает только с одним чайлдкомпонентом.Если их будет несколько , то выводиться будет первый

  @ViewChildren(FioletComponent) childComponents?:QueryList<FioletComponent>;// QueryList - это не просто массив, это обертка над массивом.
